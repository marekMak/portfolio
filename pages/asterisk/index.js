...
export const getStaticProps = async () => {
  const res = await fetch(
    "https://baro.ibasterisk.sk/wp-json/wp/v2/posts?categories=31"
  );
    ...
    <div className="container mx-auto min-h-screen h-auto flex flex-col items-center py-20">
      ...
      >
        {filteredData.map((item) => (
          <Link
            key={item.id}
            href={`/asterisk/${item.id}`}
            className="bg-[rgba(65,47,123,0.15)] h-max z-50 rounded-lg px-6 py-8 flex justify-between items-center group cursor-pointer hover:bg-[rgba(89,65,169,0.15)] transition-all duration-300"
          >
            <div
