    ...
    return (
      <div className='container mx-auto min-h-screen h-auto flex flex-col items-center gap-x-6 py-20'>
        <motion.h1
          ...
          {data.map((item, index)=>{
            return(
              <Link href={`./siete/${item.id}`} as={`/siete/${item.id}`} className='bg-[rgba(65,47,123,0.15)] h-max rounded-lg px-6 py-8 flex flex-1 sm:flex-col gap-x-6 sm:gap-x-0 group cursor-pointer hover:bg-[rgba(89,65,169,0.15)] transition-all duration-300'>
              {/* icon */}
